      ...
      <div className="m-5 w-full p-4 max-w-3xl mx-auto  border-black border-2 rounded-lg">
        ...
        <div className="">
          <div className="space-y-4">
            ...
            <div className="w-full bg-grey h-1 " ></div>
            <div>
              <div>Analysis:-</div>
  ...
  return (
    <div className="w-full p-4 max-w-3xl mx-auto border-black border-2 rounded-lg mt-10">
          ...
          <div>
            {questions[currentQuestion].options.map((option, index) => (
              <div key={index} className="flex items-center space-x-2 ml-4">
                <input type="radio" id={`option-${index}`} name="option" value={index.toString()} className="h-4 w-4 text-blue-600 focus:ring-blue-500" />
                <label htmlFor={`option-${index}`} className="ml-2 block text-sm text-gray-900">{option}</label>
              </div>
      ...
      <div className='flex justify-between p-4'>
        <div onClick={handleBefore} className="mr-5 border-black border-2 rounded-lg p-2 hover:bg-light_yellow transition-all duration-300">
          {currentQuestion === questions.length - 1 ? 'Submit' : 'Previous'}
        </div>

        <div onClick={handleNext} className="  border-black border-2 rounded-lg p-2 hover:bg-light_yellow transition-all duration-300 ">
